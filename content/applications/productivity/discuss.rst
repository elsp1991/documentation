...
a new direct message, click the :icon:`fa-plus` :guilabel:`(plus)` icon next to :guilabel:`Direct Messages`.
...
for the conversation, or click :icon:`fa-phone` :guilabel:`Start a Call` to begin a :ref:`meeting <discuss/meetings>`. Click
...
:icon:`fa-times` :guilabel:`(times)` icon next to the name of the direct message conversation. This does not delete the
...
Once a meeting has started, the buttons on the bottom of the call screen can be used to
...
options including :guilabel:`Notification` :guilabel:`Voice & Video`, and :guilabel:`Canned Responses`.
...
By default, the :guilabel:`Notification` field is set as :guilabel:`Handle by Emails`. With this setting enabled, Odoo sends a notification email if a message is sent from the chatter, a note is sent with an @ mention (from Chatter), or a notification is sent for a record the user follows. Something that triggers a notification is the changing of the stage (if an email
...
channels, or start a new message, select the :icon:`fa-comments` :guilabel:`(comments)` icon in the upper right corner of any page
in Odoo.
